...
class ExpressionEvaluator:
    """Shunting-yard algorithm for evaluating arithmetic expressions."""
    def __init__(self):
        self.precedence = {
            '+': 1, '-': 1,
            '*': 2, '/': 2,
...
class ModernCalculator(QMainWindow):
    ...
    def init_ui(self):
        ...
        for text, row, col, rowspan, colspan, btn_type in buttons:
            ...
            # Add tooltips for functions
            if text in ("√", "x²", "1/x", "xʸ", "%"):
                btn.setToolTip(f"Apply {text} to current value")
            elif text in ("M+", "M-", "MR", "MC"):
                btn.setToolTip(f"Memory {text[1:] if text != 'MC' else 'Clear'}")
        ...
        # Store title bar reference for dragging
    ...
    def animate_button(self, button):
        """Scale button down and up quickly."""
        anim = QPropertyAnimation(button, b"geometry")
        anim.setDuration(100)
        anim.setEasingCurve(QEasingCurve.OutQuad)
        original_geo = button.geometry()
        scaled_geo = QRect(original_geo.x() + 5, original_geo.y() + 5,
                           original_geo.width() - 10, original_geo.height() - 10)
        anim.setKeyValueAt(0, original_geo)
        anim.setKeyValueAt(0.5, scaled_geo)
        anim.setKeyValueAt(1, original_geo)
        anim.start()
